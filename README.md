...
Built using:
- Spring AOP (`@Around` pointcut on internal HTTP client bean)
- Java Reflection (field access across class hierarchy to find OkHttpClient pool)
- OkHttp native interceptor chain (`okhttp3.Interceptor`)
- Spring Boot autoconfiguration (`@AutoConfiguration` + `AutoConfiguration.imports`)
- `BeanPostProcessor` for zero-config RestTemplate support
- MDC correlation ID propagation for distributed tracing
- `@ConditionalOnClass` / `@ConditionalOnProperty` for safe optional activation
